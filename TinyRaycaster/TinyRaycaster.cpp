...
struct Texture
{
  // instead of creating a dynamic collection of pixels, we'll return a pair of
  // iterators
  struct iter;
  struct sentinel;
  ...
  inline column get_scaled_column(size_t textureId, size_t x,
                                  size_t height) const
  {
...
inline size_t texture_x(float hit_x, float hit_y, const Texture& walls)
{
  auto x = frac(hit_x);
  auto y = frac(hit_y);
  int tex = (std::abs(y) > std::abs(x)) ? size_t(y * walls.size)
                                        : size_t(x * walls.size);
  if (tex < 0)
    tex += walls.size;
...
void draw_map(FrameBuffer& fb, const Texture& walls, const Map& map,
              const std::vector<Sprite>& sprites, size_t cell_w, size_t cell_h)
{
  ...
  { // draw the map itself
    for (size_t i = 0; i < map.w; i++)
    {
      ...
      fb.drawRectangle(
          rect_x, rect_y, cell_w, cell_h,
          walls.get_pixel(0, 0, texid)); // the color is taken from the upper
  ...
  { // show the monsters
    fb.drawRectangle(size_t(sprites[i].x * cell_w - monster_size / 2),
                     ...
                     size_t(monster_size), size_t(monster_size),
                     color(255, 0, 0));
  }
}
                 ...
                 const std::array<float, W / 2>& depth_buffer)
{
  ...
  while (sprite_dir - player.a > Pi)
    sprite_dir -=
        2 * Pi; // remove unncesessary periods from the relative direction
  while (sprite_dir - player.a < -Pi)
    sprite_dir += 2 * Pi;

  size_t sprite_screen_size = std::min<size_t>(
      2000, static_cast<size_t>(H / sprite.playerDist)); // screen sprite size
  // do not forget the 3D view takes only a half of the framebuffer, thus fb.w/2
  // for the screen width
  int h_offset = (sprite_dir - player.a) * (W / 2) / (player.fov) +
                 (W / 2) / 2 - sprite_screen_size / 2;
  ...
  for (size_t i = 0; i < sprite_screen_size; ++i)
  {
    ...
    auto column = tex.get_scaled_column(
        sprite.texId, i * tex.size / sprite_screen_size, sprite_screen_size);
...
void render(const GameState& gs, FrameBuffer& fb)
{
  ...
  for (size_t i = 0; i < W / 2; ++i)
  {
    ...
    for (auto t = 0.f; t < max_ray; t += 0.01f)
    {
      ...
      auto tex_id =
          map.get(map_x, map_y); // our ray touches a wall, so draw the vertical
